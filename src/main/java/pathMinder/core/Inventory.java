 ...
 * Unlike most containers, an Inventory's capacity isn't fixed and is dictated by the player's strength;
...
class Inventory extends ItemGroup {
	...
	Inventory(Character holder) {
		super(String.format("%s's inventory", holder.getName()), Float.MAX_VALUE, Float.MAX_VALUE, Integer.MAX_VALUE);
		this.holder = holder;
	}
